...
bool InitPassSecond()
{
    QueryPerformanceCounter(&s_lastTime_);
}
float GetPassedSeconds(unsigned long elapsedFrames = 1)
{
  LARGE_INTEGER currentTime;

  QueryPerformanceCounter(&currentTime);

  float seconds =  ((float)currentTime.QuadPart - (float)s_lastTime_.QuadPart) / (float)m_ticksPerSecond.QuadPart;
