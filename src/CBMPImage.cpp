...
void Bitmap::SetColorKey(unsigned char Red, unsigned char Green, unsigned char Blue)
{
    // For all the pixels that correspond to the specifed color,
	// set the alpha channel to 0 (transparent) and reset the other
	// ones to 255.
	unsigned long Count = width * height * 3;
	for (unsigned long i = 0; i<Count; i+=3)
	{
		if ( (data[i]==Red) && (data[i+1]==Green)
			  && (data[i+2]==Blue) )
			data[i+3] = 0;
		else
			data[i+3] = 255;
